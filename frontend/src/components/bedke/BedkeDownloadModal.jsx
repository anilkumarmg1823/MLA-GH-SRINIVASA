...
import { downloadBedkeExcel, downloadBedkePdf } from "@/lib/exportBedke";
...
export default function BedkeDownloadModal({
  open,
  onClose,
  currentGp,
  currentVillage,
  ...
  useEffect(() => {
    ...
    (async () => {
      let rows = [];
      if (scope === "all") rows = await getAllBedke();
      else if (scope === "gp") {
        if (!gp) {
          if (!cancelled) setPreviewRows([]);
          return;
        }
        rows = await getBedkeForGp(gp);
      } else {
        if (!gp || !village) {
          if (!cancelled) setPreviewRows([]);
          return;
        }
        rows = await getBedkeForVillage(gp, village);
      }
      if (approach !== "all") {
        rows = rows.filter((r) => r.approach === approach);
      }
  ...
  }, [scope, gp, village, approach, dateFrom, dateTo, open]);
  ...
  const handleDownload = () => {
    ...
    if (approach !== "all") label += `_${approach}`;
    if (dateFrom || dateTo) {
    ...
    if (format === "excel") {
      downloadBedkeExcel(previewRows, lang, `bedke-${label}-${stamp}.csv`);
    } else {
      const title =
        lang === "kn" ? `ಬೇಡಿಕೆಗಳು ವರದಿ — ${label}` : `Demands Report — ${label}`;
      downloadBedkePdf(previewRows, lang, title);
    }
    onClose();
  };
                ...
                <table className="min-w-full text-left text-xs sm:text-sm">
                  ...
                  <tbody>
                    {previewRows.map((row) => (
                      ...
                      >
                        <td className="px-3 py-2.5 text-[var(--dash-text)] font-semibold whitespace-nowrap">
                          {row.name}
                        </td>
                        ...
                        <td className="px-3 py-2.5 text-[var(--dash-text-80)] max-w-[220px]">
